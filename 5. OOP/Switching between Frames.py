...
class ConverterFrame(ttk.Frame):
    def __init__(self, container, unit_from, converter):
        ...
        # field options
        self.options = {"padx": 5, "pady": 0, "ipadx": 5}

        # temperature label
        self.temperature_label = ttk.Label(self, text=self.unit_from)
        self.temperature_label.grid(
            column=0, row=0, sticky='w',  **self.options)  # type: ignore

        # temperature entry
        self.temperature = tk.StringVar()
        self.temperature_entry = ttk.Entry(self, textvariable=self.temperature)
        self.temperature_entry.grid(
            column=1, row=0, sticky='w', **self.options)  # type: ignore
        self.temperature_entry.focus()

        # button
        self.convert_button = ttk.Button(self, text='Convert')
        self.convert_button.grid(
            column=2, row=0, sticky='w', **self.options)  # type: ignore
        self.convert_button.configure(command=self.convert)

        # result label
        self.result_label = ttk.Label(self)
        self.result_label.grid(
            row=1, columnspan=3, **self.options)  # type: ignore
